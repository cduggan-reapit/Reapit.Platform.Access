...
using Reapit.Platform.Access.Api.Controllers.Synchronisation.Organisations.V1.Examples;
...
using Reapit.Platform.Access.Core.UseCases.OrganisationUsers.CreateOrganisationUser;
using Reapit.Platform.Access.Core.UseCases.OrganisationUsers.DeleteOrganisationUser;
...
public class OrganisationsController : ReapitApiController
{
    ...
    {
        var request = new CreateOrganisationUserCommand(id, userId);
        var organisation = await _mediator.Send(request);
        return NoContent();
    }
    ...
    {
        var request = new DeleteOrganisationUserCommand(id, userId);
        var organisation = await _mediator.Send(request);
        return NoContent();
    }
}